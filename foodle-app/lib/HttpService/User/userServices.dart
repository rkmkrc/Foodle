...
Future<void> getUserPicture(String userIDtoBeFollowed) async {
  var sharedPreferences = await SharedPreferences.getInstance();
  String userID = sharedPreferences.getInt("userID").toString();
  String accessToken = sharedPreferences.getString("accessToken").toString();

  var url =
      Uri.parse('http://172.16.3.237:3000/users/$userID/profile_photo.jpg');
  var response = await http.get(
    url,
  );
  if (response.statusCode == 200) {
    print("-------------getUserPicture SUCCESS--------------");
  } else {
    print("-------------getUserPicture UNSUCCESS--------------");
  }
